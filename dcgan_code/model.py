...
class DCGAN(object):
    def __init__(self, sess, image_size=64, is_crop=False,
                 ...
                 checkpoint_dir='/Users/Joe/Desktop/gp/Application_GUI/dcgan_code/checkpoints/celebA_checkpoint', lam=0.2):
        """
    ...
    def complete(self, config):
        def make_dir(name):
            ...
            if not os.path.exists(p):
                os.makedirs(p)
        make_dir('hats_imgs')
        make_dir('completed_' + config.maskType)
        make_dir('logs')
        ...
        for idx in xrange(0, batch_idxs):
            ...
            for i in xrange(config.nIter):
                ...
                if i % config.outInterval == 0:
                    ...
                    if i == config.nIter - 1:
                      for mn in range (batchSz):
                        summ_loss += loss[mn]
                        min_loss = min(min_loss,loss[mn])
                        max_loss = max(max_loss , loss[mn])
                      if idx == batch_idxs-1:
                        print("Avg Loss" ,(summ_loss/nImgs) ,"Max loss", max_loss, "Min loss",min_loss)
                    ...
                    imgName = os.path.join(config.outDir,
                                           'completed_' + config.maskType + '/{}_{:04d}.png'.format(idx, i))
